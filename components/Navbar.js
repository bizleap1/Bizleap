...
export default function Navbar() {
  ...
  const isCreatorsPage = router.pathname === "/creators"
  ...
  return (
    <nav
      className={`fixed w-full z-50 shadow-md transition-colors duration-500 ${
        scrolled ? "bg-black text-white border-b border-white/10" : "bg-transparent text-white"
      }`}
    >
                          ...
                          {serviceSubmenu.map((subItem) => (
                            <div key={subItem.name} className="relative group/sub">
                              {subItem.subItems ? (
                                ...
                                >
                                  <div 
                                    className="flex items-center justify-between px-4 py-3 hover:bg-gray-800/50 transition-colors duration-200 cursor-pointer"
                                  ...
                                  <AnimatePresence>
                                    {staffingOpen && (
                                        <motion.div
                                          className="absolute top-0 left-full ml-0.5 w-max min-w-[200px] bg-black/95 backdrop-blur-sm rounded-lg shadow-lg py-2 z-50"
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            className={`md:hidden px-4 pt-2 pb-4 space-y-2 overflow-y-auto max-h-[calc(100vh-64px)] ${
                                ...
                                <div>
                                  <div className="flex items-center">
                                    ...
                                    <button
                                      ...
                                      onClick={() => setStaffingOpen(!staffingOpen)}
                                    >
                                      <FiChevronDown className={`transition-transform duration-200 ${staffingOpen ? 'rotate-180' : ''}`} />
                                    </button>
                                  </div>
                  ...
                  <div>
                    <div className="flex items-center">
                      <Link
                        ...
                        className="flex-1 text-left px-4 py-2 font-medium hover:bg-gray-800 rounded"
                        onClick={() => {
                          if (link.href !== "#") setMenuOpen(false)
                        }}
                      >
                        {link.name}
                      </Link>
